...
def main() -> int:
    # The agent-codex wrapper owns the user-level Trellis dispatcher. Keep this
    # project hook portable for ordinary Codex sessions, but avoid duplicate
    # workflow-state output when the wrapper explicitly marks its dispatch.
    if os.environ.get("AGENT_CODEX_TRELLIS_USER_DISPATCH") == "1":
        return 0
