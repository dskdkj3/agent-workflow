...
<body>
  ...
  <script>
    ...
    const usage = item => item.value == null ? 'unknown' : item.status + ': input=' + item.value.input_tokens + ', cached=' + item.value.cached_input_tokens + ', output=' + item.value.output_tokens + ', reasoning=' + item.value.reasoning_output_tokens;
    ...
    const routing = item => '<div>Routing: ' + esc(item.source) + ' / ' + esc(item.task_class ?? 'unknown') + '</div>' + (item.residual_burden ? '<div class="muted">Burden: ' + esc(item.residual_burden) + '</div>' : '') + (item.why_lower_cost_route_is_insufficient ? '<div class="muted">Lower-cost rejection: ' + esc(item.why_lower_cost_route_is_insufficient) + '</div>' : '') + (item.upgrade_trigger ? '<div class="muted">Upgrade trigger: ' + esc(item.upgrade_trigger) + '</div>' : '');
    const agents = items => '<ul>' + items.map(a => '<li><strong>' + esc(a.role) + '</strong> <code>' + esc(a.id) + '</code> <span class="' + esc(a.status) + '">' + esc(a.status) + '</span><div class="muted">' + esc(a.model) + ' / ' + esc(a.reasoning_effort) + ' / ' + esc(a.profile) + ' · ' + esc(duration(a.duration_ms)) + '</div>' + routing(a.routing) + '<div>Fast: ' + esc(fast(a.fast)) + '</div><div>Usage: ' + esc(usage(a.usage)) + '</div><div>Quota equivalent: unknown</div>' + agents(a.children) + '</li>').join('') + '</ul>';
    const render = trace => {
